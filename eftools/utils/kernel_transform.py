...
from eftools.units import SI

__all__ = ["shift_spectral_to_kernel", "kernel_to_spectral"]

def shift_spectral_to_kernel(J, UNITS: SI, T: float, P: int, n: int, wmax: float, eps: float, tvec: np.ndarray):
...
def shift_lambda_to_kernel(Lambda, UNITS: SI, T: float, P: int, n: int, wmax: float, eps: float, tvec: np.ndarray):
    """Convert the classical spectral density to the memory-friction kernel for a non-centroid normal mode.
